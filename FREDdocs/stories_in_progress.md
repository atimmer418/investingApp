...
Tracks stories pulled into active work via the triage skill (`--story=<ID>` or `--mode=override`). One entry per unique story ID. To mark a story complete, use `/triage --done=<ID>`.
