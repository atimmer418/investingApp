...
- **Canonical as-you-type search pipe** (FRED-186, applied to lump-sum-investment, portfolio-customize, and stockselection stock searches): never call the async search directly from the input handler. Declare `searchInput$ = new Subject<string>()`; the `(ionInput)`/input handler calls `this.searchInput$.next(value)`; subscribe ONCE in `ngOnInit` with `debounceTime(300)` → `distinctUntilChanged()` → `switchMap(term => term.length < 2 ? of(empty) : forkJoin([equity$, etf$]))` → `takeUntil(this.destroy$)`, moving result assignment into the subscription. `switchMap` cancellation is load-bearing — it kills the stale-result race (a slow earlier response can no longer overwrite a newer one). Put `catchError(() => of(empty))` INSIDE the switchMap (on each inner request and/or the forkJoin), never on the outer stream, so a failed request emits a safe empty payload and the Subject pipe survives for the next keystroke. When the pipe owns the 300ms window, REMOVE the native `debounce="300"` attribute from the ion-searchbar (otherwise it double-debounces to ~600ms). Mirrors the existing `app.component.ts` idiom; do NOT extract a shared utility/directive for a handful of call sites (rejected as over-abstraction).
